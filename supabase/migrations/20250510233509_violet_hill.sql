/*
  ...
  1. Changes
    - Adds proper constraints and relationships between tables
    - Updates RLS policies with proper permission checks
    - Ensures no duplicate policy creation
...
DO $$ BEGIN
    DROP POLICY IF EXISTS "Users can insert their own libraries" ON user_libraries;
    DROP POLICY IF EXISTS "Users can view their own libraries" ON user_libraries;
    DROP POLICY IF EXISTS "Users can update their own libraries" ON user_libraries;
    DROP POLICY IF EXISTS "Users can delete their own libraries" ON user_libraries;
EXCEPTION
...
CREATE POLICY "Users can view their own libraries"
  ...
  USING (
    ...
    EXISTS (
      ...
      WHERE shared_library_permissions.library_id = user_libraries.id 
...
-- Drop existing policies for shared_library_permissions if they exist
DO $$ BEGIN
    DROP POLICY IF EXISTS "Users can create sharing entries" ON shared_library_permissions;
    DROP POLICY IF EXISTS "Users can view their sharing permissions" ON shared_library_permissions;
    DROP POLICY IF EXISTS "Users can update their sharing entries" ON shared_library_permissions;
    DROP POLICY IF EXISTS "Users can delete their sharing entries" ON shared_library_permissions;
EXCEPTION
    WHEN undefined_object THEN null;
END $$;
...
-- Drop existing policies for shared_clipboard_items if they exist
DO $$ BEGIN
    DROP POLICY IF EXISTS "Users can insert their own items" ON shared_clipboard_items;
    DROP POLICY IF EXISTS "Users can view items they have access to" ON shared_clipboard_items;
    DROP POLICY IF EXISTS "Users can update items with permission" ON shared_clipboard_items;
    DROP POLICY IF EXISTS "Users can delete items with permission" ON shared_clipboard_items;
EXCEPTION
    WHEN undefined_object THEN null;
END $$;
